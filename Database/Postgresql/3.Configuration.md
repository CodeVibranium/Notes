...
2. By default when a connection is established to db via client the connection is not broken until user logsout, but some libraries such as `prisma` tend to end the connection once a db is resolved
...
- can be specifiesd in human readable format.
- Rotate log files after they reach 100MB
  - `log_rotation_size = 100MB`
- if `logging_collector` is set `on` log_rotation_size will be applied tp that dir.
...
- when `log_min_duration_statement` is set to non-negative value postgres qiull log all queriies which took kore than specified time.
